...
function M.describe_instance_details(instance_id, on_result)
    handler.aws_group_command({
        { "ec2", "describe-instance-status --query 'InstanceStatuses[] | [0]' --instance-ids  " .. instance_id },
        { "ec2", "describe-instances --query 'Reservations[].Instances[] | [0]' --instance-ids " .. instance_id },
    }, on_result)
end
...
function M.describe_instance_monitoring(instance_id, current_time, hours, interval, on_result)
    ...
    handler.aws_command(
        "cloudwatch",
        "get-metric-data --metric-data-queries " ..
...
function M.start_instance(instance_id, on_result)
    handler.aws_command("ec2", "start-instances --instance-ids " .. instance_id, on_result)
end
...
function M.stop_instance(instance_id, on_result)
    handler.aws_command('ec2', 'stop-instances --instance-ids ' .. instance_id, on_result)
end
...
function M.terminate_instance(instance_id, on_result)
    handler.aws_command('ec2', 'terminate-instance --instances-ids ' .. instance_id, on_result)
end
...
function M.describe_target_groups(on_result)
    handler.aws_command("aws", "elbv2 describe-target-groups", on_result)
end
